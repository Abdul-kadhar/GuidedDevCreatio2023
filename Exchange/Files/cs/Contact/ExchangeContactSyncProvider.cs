namespace Terrasoft.Sync.Exchange
{
	...
	public class ExchangeContactSyncProviderImpl : ExchangeSyncProvider
	{
		...
		private IEnumerable<LocalItem> GetNotSyncedContacts(SyncContext context, SyncItemSchema primarySchema) {
			...
			if (!_userConnection.GetIsFeatureEnabled("SwagelokFiltersEnabled")) {
				select.And("Contact", "Id").Not().Exists(new Select(userConnection)
					.Column("LocalId")
		...
		private void AddExportFiltersBySettings(SyncContext context, EntitySchemaQuery esq) {
			...
			if (_userConnection.GetIsFeatureEnabled("SwagelokFiltersEnabled")) {
				var swagelokFilters = new EntitySchemaQueryFilterCollection(esq, LogicalOperationStrict.Or) {
					esq.CreateFilterWithParameters(FilterComparisonType.IsNotNull, "Owner")
				};
				var swagelokType = new Guid("ff1dccc6-8012-44a8-b668-21a1bdcab196");
				swagelokFilters.Add(esq.CreateFilterWithParameters(FilterComparisonType.Equal, false, "Type", swagelokType));
				esq.Filters.Add(swagelokFilters);
		...
		public override IEnumerable<IRemoteItem> EnumerateChanges(SyncContext context) {
			...
			if (!UserSettings.ImportContacts || _userConnection.GetIsFeatureEnabled("SwagelokFiltersEnabled")) {
				return result;
			}

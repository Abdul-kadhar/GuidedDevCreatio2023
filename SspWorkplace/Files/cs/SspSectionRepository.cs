namespace Terrasoft.Configuration.Section
{
	...
	public class SspSectionRepository : SectionRepository
	{
		...
		private IEnumerable<Guid> _sysModuleEntitiesInPortal;

		protected IEnumerable<Guid> SysModuleEntitiesInPortal {
			get { return _sysModuleEntitiesInPortal ?? (_sysModuleEntitiesInPortal = GetSysModuleEntitiesInPortal()); }
			set { _sysModuleEntitiesInPortal = value; }
		}
		...
		private void FillSysModuleEntityInPortal(Guid sysModuleEntityId, Guid sspId) {
			if (IsRegisteredAsSsp(sysModuleEntityId)) {
				return;
			}
			...
			entity.Save();
			SysModuleEntitiesInPortal = null;
		...
		private IEnumerable<Guid> GetSectionNonAdministratedByRecordsEntityIds(Section section) {
			...
			IEnumerable<Guid> moduleEntityIds = FilterSections(relatedEntityIds);
		...
		/// <summary>
		/// Loads list of modules in portal. 
		/// </summary>
		/// <returns>List of modules</returns>
		private IEnumerable<Guid> GetSysModuleEntitiesInPortal() {
			var result = new List<Guid>();
			var select = new Select(UserConnection)
				.Column("SysModuleEntityId")
				.From("SysModuleEntityInPortal") as Select;
			select.ExecuteReader(reader => {
				result.AddIfNotExists(reader.GetColumnValue<Guid>("SysModuleEntityId"));
			});
			return result;
		}
		...
		protected void SetSchemasAdministratedByRecords(IEnumerable<Guid> entityIds) {
		...
		protected override void SetSectionsByTypeFilters(Select select, SectionType type) {
			...
			select.Where("SysModule", "Type").IsEqual(Column.Parameter(SectionType.General))
				.And().Not().Exists(
		...
		protected IEnumerable<string> GetEntitySchemaCaptions(IEnumerable<Guid> entityIds) {
			var watch = Stopwatch.StartNew();
		...
		protected IEnumerable<Guid> GetNonAdministratedByRecordsEntityIds(IEnumerable<Guid> entityIds) {
			var watch = Stopwatch.StartNew();
		...
		/// <summary>
		/// Returns not administrated by operations <see cref="Entity"/> identifier collection from <paramref name="entityIds"/>.
		/// </summary>
		/// <param name="entityIds"><see cref="Entity"/> identifier collection.</param>
		/// <returns><see cref="Entity"/> identifiers collection.</returns>
		protected IEnumerable<Guid> GetNonAdministratedByOperationsEntityIds(IEnumerable<Guid> entityIds) {
			var watch = Stopwatch.StartNew();
			LogDebugStart("GetNonAdministratedByOperationsEntityIds", watch);
			var result = new List<Guid>();
			foreach (var entityId in entityIds) {
				LogDebug($"GetNonAdministratedByOperationsEntityIds start foreach EntitySchemaManager.GetItemByUId {entityId}", watch);
				var schema = EntitySchemaManager.GetItemByUId(entityId);
				LogDebug($"GetNonAdministratedByOperationsEntityIds end foreach EntitySchemaManager.GetItemByUId {entityId}", watch);
				LogDebug($"GetNonAdministratedByOperationsEntityIds start dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
				var schemaIsAdministratedByOperations = _dbSecurityEngine.GetIsEntitySchemaAdministratedByOperations(schema.Name);
				LogDebug($"GetNonAdministratedByOperationsEntityIds end dbSecurityEngine.GetIsEntitySchemaAdministratedByRecords {schema.Name}", watch);
				if (!schemaIsAdministratedByOperations) {
					result.AddIfNotExists(entityId);
				}
			}
			LogDebugEnd("GetNonAdministratedByOperationsEntityIds", watch);
			return result;
		}
		...
		/// Filters by sections.
		/// </summary>
		/// <param name="relatedEntityIds"><see cref="Entity"/> identifier collection.</param>
		/// <returns><see cref="Entity"/> identifiers collection.</returns>
		protected virtual IEnumerable<Guid> FilterSections(IEnumerable<Guid> relatedEntityIds) {
			var allSections = GetAll();
			return relatedEntityIds.Where(
				 (e) => {
		...
		protected bool IsRegisteredAsSsp(Guid sysModuleEntityId) {
			return SysModuleEntitiesInPortal.Any(m => m == sysModuleEntityId);
		}
